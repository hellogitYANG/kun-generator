...
import com.kunkun.maker.template.enums.FileFilterRangeEnum;
import com.kunkun.maker.template.enums.FileFilterRuleEnum;
import com.kunkun.maker.template.model.*;
...
public class TemplateMaker {
    ...
    public static long makeTemplate(Meta newMeta, String originProjectPath, TemplateMakerFileConfig templateMakerFileConfig, TemplateMakerModelConfig templateMakerModelConfig, TemplateMakerOutputConfig templateMakerOutputConfig, Long id) {
        ...
        // 一、输入信息
        // 输入文件信息，获取到项目根目录（工作空间第一个目录，可以完善配置的追加能力）
        ...
        // 如果已有 meta.json 文件，说明不是第一次制作，则在 meta 基础上进行修改
    ...
    private static Meta.FileConfig.FileInfo makeFileTemplate(File inputFile, TemplateMakerModelConfig templateMakerModelConfig,
        ...
        if (!hasTemplateFile) {
            if(contentEquals){
                ...
                // 没有模板文件，不一致，需要挖坑，生成模板文件
                FileUtil.writeUtf8String(newFileContent, fileOutputAbsolutePath);
            }

        }else if (!contentEquals) {
            // 有模板文件，不一致，增加了新坑，生成模板文件
            FileUtil.writeUtf8String(newFileContent, fileOutputAbsolutePath);
        }
    ...
    private static List<Meta.FileConfig.FileInfo> distinctFiles(List<Meta.FileConfig.FileInfo> fileInfoList) {
        //1.有分组的，以组为单位划分
        //{"group":"a",files[1,2]},{"group":"a",files[2,3]},{"group":"b",files[4,5]}
        //{"group":"a",files[[1,2],[2,3]]},{"group":"b",files[4,5]}
        ...
        // 2. 同组内的文件配置合并
        // {"group":"a",files[[1,2],[2,3]]}
        // {"group":"a",files[1,2,2,3]}
        // {"group":"a",files[[1,2,3]}
        Map<String, Meta.FileConfig.FileInfo> groupKeyMergedFileInfoMap = new HashMap<>();
        for (Map.Entry<String, List<Meta.FileConfig.FileInfo>> entry : groupKeyFileInfoListMap.entrySet()) {
            List<Meta.FileConfig.FileInfo> tempFileInfoList = entry.getValue();
            List<Meta.FileConfig.FileInfo> newFileInfoList = new ArrayList<>(tempFileInfoList.stream().flatMap(fileInfo -> fileInfo.getFiles().stream())//[1,2,2,3]
                    .collect(Collectors.toMap(Meta.FileConfig.FileInfo::getOutputPath, o -> o, (e, r) -> r)//[1,2,3]
                    ).values());
